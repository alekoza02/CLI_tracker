...
class PlotManager:
  ...
  def set_plot_data(self, data, metadata="", channels=1):
    if len(data) > 2 and not self.analytics['tracked_type'] is None:
      ...
      if self.analytics['tracked_type'] == 'elements':
        self.original = data[-self.analytics['x_tracked']:]
  
      elif self.analytics['tracked_type'] == 'seconds':
        x_last = data[-1][0]
...
if __name__ == "__main__":
  ...
  parser.add_argument("-t", default="seconds")
  parser.add_argument("-n", default="30")
  parser.add_argument("-i", default="y")
  parser.add_argument("-u", default="0.5")
  ...
  terminal.interpol_plot.track_last_tail(int(args.n), args.t)
  ...
  try:
    while 1:
      ...
      if args.i == "y":
        ...
        x1, y1, x2, y2 = 1, 1, terminal.w - 2, int(4 * terminal.h / 5) - 2
        terminal.data_plot.set_boundaries(x1, y1, x2 - x1, y2 - y1)
      ...
      if args.i == "y":
        ...
        terminal.insert_colored_frame(0, 0, terminal.w, int(4 * terminal.h / 5) - 1, "Plot section", (2, 3, 4))
      ...
      terminal.insert_string_horizontal(f"Reporting '{terminal.args.f}', with {terminal.data_plot.analytics["len"]} entries.", 2, int(4 * terminal.h / 5) + 2)
      ...
      terminal.insert_string_horizontal(f"{'Rendering FPS:':<27} {1 / rendering_fps:.2f}", 2, int(4 * terminal.h / 5) + 3)
      terminal.insert_string_horizontal(f"{'Capped FPS:':<27} {1 / capped_fps:.2f}", 2, int(4 * terminal.h / 5) + 4)
      terminal.insert_string_horizontal(f"{'File readings:':<27} {terminal.file_manager.readings:.0f}", 2, int(4 * terminal.h / 5) + 5)

      terminal.flip(1 / float(args.u))
